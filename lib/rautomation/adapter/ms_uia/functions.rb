          ...
          def control_hwnd(window_hwnd, locators)
            case
              when locators[:id]
                ...
                raise UnknownElementException.new("#{locators[:id]} does not exist") if hwnd == 0
                hwnd
              when locators[:point]
                uia_control = UiaDll::element_from_point(locators[:point][0], locators[:point][1])
                hwnd = UiaDll::current_native_window_handle(uia_control) # return HWND of UIA element
                raise UnknownElementException.new("#{locators[:point]} does not exist") if hwnd == 0
                hwnd
              else
                ...
                raise UnknownElementException.new("Element with #{locators.inspect} does not exist") if (hwnd == 0) or (hwnd == nil)
                hwnd
            end
          end
